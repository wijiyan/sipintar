...
class DataIbu extends Auth_Controller {
	public function __construct() {
		parent::__construct();
		$this->load->model('M_DataIbu');
	}
	...
	public function index() {
		...
		$data['modal_tambah_dataibu'] = show_my_modal('modals/modal_tambah_dataibu', 'tambah-dataibu', $data);

		$this->template->views('DataIbu/dataibu', $data);
	...
	public function tampil1($cari = '', $isi = '') {
		if($isi == ''){
			...
			$data['dataIbu'] = $this->M_DataIbu->select_all($cari, $isi);
		}
	...
	public function simpan() {
		...
		$nik = $data['nik'];
		$cek_nik = $this->M_DataIbu->cek_nik($nik);
		...
		{
			$result = $this->M_DataIbu->insert($data);
	...
	public function prosesUpdate() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$result = $this->M_DataIbu->update($data);
